...
# Precompiled header for the driver-injected intrinsics.  The AIE driver
# -includes 67k lines of aie2p intrinsics into every TU, which Sema re-parses per
# kernel: an EMPTY TU costs 2.14 s on aie2p against 0.45 s on aie2, 2.02 s of it
# in ParseDeclarationOrFunctionDefinition (-E is 0.19 s, so it is the parse).
# Parsing it once and passing it back with -mno-vitis-headers takes
# aie2p/mha.cc 5.28 s -> 2.80 s, 251.6 MB -> 205.6 MB RSS, object byte-identical.
# The PCH source is an EMPTY header, so it holds exactly what it replaces.  The
# key covers only this function's flags, never a caller's -I/-D: a conflicting
# define is a hard clang error naming the PCH, and the retry below falls back to
# an ordinary compile.  Set AIE_KERNEL_PCH=0 to disable.
...
def _warn_launcher_pch_interaction(launcher: str) -> None:
    """ccache silently declines to cache a compile that uses -include-pch.
