...
static bool checkAndPrintBufferOverlap(ArrayRef<BufferOp> sortedBuffers,
  ...
  for (size_t i = 0; i < sortedBuffers.size(); ++i) {
    ...
    // of. Comparing only against sortedBuffers[i - 1] is sound only while no
    // pair is exempt: a small same-group buffer overlaid on a large one leaves
    // a third buffer adjacent only to the small one, so its overlap with the
    // large one is never examined.
    //
    ...
    // laid out sequentially and must not overlap; a grouped buffer overlapping
    // an UNGROUPED one still is. Both groups must be present for the exemption,
...
static bool basicAllocation(TileOp tile) {
  ...
  // Pinned buffers enter the layout as themselves. The rest are grouped into
  // alloc units -- one layout entry each, sized by the largest group it
  // overlays -- whose assigned base is distributed across those groups below.
  SmallVector<BufferOp> unpinnedBuffers;
  for (auto buffer : allBuffers_on_tile)
    if (!buffer.getAddress())
      unpinnedBuffers.push_back(buffer);
  SmallVector<AllocUnit> units = buildAllocUnits(unpinnedBuffers);
  ...
  for (const AllocUnit &unit : units) {
        ...
        if (buffer.getAligned())
          unitAlignBits = std::max(
              unitAlignBits, getRequiredAlignBits(buffer, tileAlignBitWidth,
                                                  maxVecAlignBits));
    layout.push_back(
        {unit.size, unit.aligned ? unitAlignBits / 8 : 1, std::nullopt});
  }

  int64_t highWater = assignSequentialBufferAddresses(layout, address);

  for (size_t i = 0; i < allocated_buffers.size(); ++i) {
    assert(layout[i].address && "layout assigns every buffer an address");
    allocated_buffers[i].setAddress(layout[i].address.value());
  }
  ...
  for (size_t i = 0; i < units.size(); ++i) {
    const BufferAllocation &allocation = layout[allocated_buffers.size() + i];
    assert(allocation.address && "layout assigns every unit an address");
    for (const auto &group : units[i].groups) {
      int64_t offset = *allocation.address;
      for (auto buffer : group) {
