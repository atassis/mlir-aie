...
using DATATYPE_IN2 = test_utils::bfloat16_t; // w = [taps(KERNEL_SIZE) | bias-or-pad(1)]
...
int verify_dwconv1d_kernel(DATATYPE_IN1 *bufIn1, DATATYPE_IN2 *bufIn2,
  ...
  for (int r = 0; r < CHANNELS; r++) {
    ...
    for (int t = 0; t < SEQ_LEN; t++) {
      double acc = BIAS ? (double)test_utils::bfloat16_to_float(wrow[KERNEL_SIZE]) : 0.0;
      for (int p = 0; p < KERNEL_SIZE; p++)
      ...
      float ev = test_utils::bfloat16_to_float(expected);
      float hv = test_utils::bfloat16_to_float(got);
      if (std::fabs(ev - hv) > 0.05f) {
        if (errors < 10)
          std::cout << "Mismatch at channel " << r << " t " << t
                    << ": expected " << ev << ", got " << hv << std::endl;
        errors++;
      } else {
        pass++;
      }
