...
1. `dwconv1d.cc`: AIE2P kernel, pulled from [`aie_kernels/aie2p/`](../../../aie_kernels/aie2p/). Vectorized 16-outputs/iteration: two aligned loads build a 32-lane window, and `aie::shuffle_down_fill` slides a 16-wide tap window across it in-register (no unaligned loads); each tap's product is `aie::mac`'d into one `accfloat` accumulator. `aie::sliding_mul_ops` is the obvious primitive for this shape but produces wrong output (inf/nan) for bf16 on this target even at K=1, a genuine `aie_api` defect, not a misuse of the op; see the kernel's own header comment for the isolated repro. This kernel routes around it with the aligned-load + shuffle + mac path above.
