...
def build(dev, mono=False, TRIVIAL=False, relpos=False):
    ...
    if mono:
        ...
        for gi, hs in enumerate(groups):
            ...
            # k/v are RESIDENT weights: filled ONCE into the MemTile (below), split to per-head sub-fifos
            # delivered ONCE; stage A/C acquire-ONCE and hold across the N_QT loop. No replay tap, no
            # repeat_counts -> minimal MemTile DMA descriptors (the N_QT-replay repeat_counts blew >48 blocks).
        ...
        def stage_a(f_q, f_k, f_ac, k_sc):
            ek = f_k.acquire(1)          # resident k: acquire ONCE (no per-tile replay -> no MemTile
            for _ in range_(N_QT):       # repeat_counts blocks, no LPDDR re-read; the split delivers k once)
        ...
        def stage_c(f_probs, f_v, f_ctx, k_cx):
            ev = f_v.acquire(1)          # resident V: acquire ONCE (mirror of resident k in stage A)
            for _ in range_(N_QT):
