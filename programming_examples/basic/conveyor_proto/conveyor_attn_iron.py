...
def build(dev, mono=False, TRIVIAL=False, relpos=False):
    ...
    q_full_ty = np.ndarray[(N_QT * TQ * DK,), np.dtype[bfloat16]]
    ...
    of_q = ObjectFifo(q_ty, name="q", depth=2)
    of_ctx = ObjectFifo(ctx_ty, name="ctx", depth=2)
    # ADVANCING taps for q (in) / ctx (out): outer dim N_QT strides by TQ*DK (distinct tile per step).
    q_tap = TensorAccessPattern([N_QT * TQ * DK], 0, [N_QT, 1, TQ, DK], [TQ * DK, 0, DK, 1])
