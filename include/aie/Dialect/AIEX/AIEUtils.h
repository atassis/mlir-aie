...
//
// `dedupCache`/`nextId`, when provided, make repeated calls O(1) instead of the
// O(n)-per-call (= O(n^2) over a whole runtime sequence) behavior of rescanning
// every existing global for dedup and probing the symbol table to find a free
// name on each call. Callers lowering many BDs (e.g. NPU DMA lowering of a large
// multi-column program) MUST pass both, seeded once from the device's existing
// globals: `dedupCache` maps initial-value -> global; `*nextId` is the next free
// "blockwrite_data_<n>" index (one past the max already present), which the
// helper post-increments per created global. Seeding past the max guarantees
// unique names by construction (no symbol-table probe). Both are scoped to one
// device's lowering and hold raw op handles — discard them afterward.
