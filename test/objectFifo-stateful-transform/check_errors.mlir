...
// same way distribute's outputs already have to. Before this check existed,
// disagreeing input repeat_counts were never even compared: getRepeatCount()
// only ever read the (single) output fifo.
module {
  aie.device(xcve2302) {
    %tile12 = aie.tile(1, 2)
    %tile23 = aie.tile(2, 3)
